...
if (process.argv.length !== 8) {
    console.log(`                       
        Usage: node HTTP-BLAZE.js <MODE> <host> <proxies> <time> <rate> <threads> (options cookie="" postdata="" randomstring="" headerdata="")
        Usage: node HTTP-BLAZE.js GET https://example.com proxy.txt 30 64 4
        -------------------------------------------------------
        VolityStress.vip | HTTP-BLAZE v1.5
        Made for VolityStress By @tcpall                                       
    `.rainbow);
    process.exit(0);
}
...
if (cluster.isMaster){
    ...
    console.log('HTTP-BLAZE Send! HTTP-BLAZE by @tcpall');
